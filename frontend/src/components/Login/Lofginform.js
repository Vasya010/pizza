...
function Lofginform() {
  ...
  const [password, setPassword] = useState(''); // Изменено на password
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/login', {
        ...
        body: JSON.stringify({ email, password }), // Отправляем пароль, а не его хэш
      });
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-group">
              ...
              <input
                ...
                id="password"
                className="input-field"
                value={password} // Изменено на password
